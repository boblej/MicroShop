...
async def show_users_with_profiles(session: AsyncSession):
    stmt = select(User).options(joinedload(User.profile)).where(User.profile != None).order_by(User.id)
...
async def demo_get_products_though_secondary(session: AsyncSession):
...
async def get_orders_with_products_assoc(session: AsyncSession) -> list[Order]:
    stmt = (
        select(Order)
        .options(
            selectinload(Order.product_details)
...
async def demo_get_orders_with_products_with_assoc(session: AsyncSession):
    ...
    for order in orders:
        print(order.id, order.promocode, order.created_at, "products:")
        for order_product_detail in order.product_details: #type: OrderProductAssociation
            print("-", order_product_detail.product.id, order_product_detail.product.name, order_product_detail.product.price, "qty:", order_product_detail.count)  


async def create_gift_products_for_existing_order(session: AsyncSession):
    order = await get_orders_with_products_assoc(session)
    gift_product = await create_product(session, "Gift", "Gift for you", 0)
    for order in order:
        order.product_details.append(OrderProductAssociation(
            count=1,
            unit_price=0,
...
async def demo_m2m(session: AsyncSession):
    ...
    # await demo_get_products_though_secondary(session)
    await demo_get_orders_with_products_with_assoc(session)
    # await create_gift_products_for_existing_order(session)
